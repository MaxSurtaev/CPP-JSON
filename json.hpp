...
public:
  ...
  static JSON Make(Class type) { return JSON(type); }
  ...
  template <typename T> void push_front(T arg) {
    SetType(Class::Array);
    Internal.List->emplace_front(arg);
  }

  template <typename T, typename... U> void push_front(T arg, U... args) {
    push_front(args...);
    push_front(arg);
  }
  ...
  bool remove(const string &key) {
    if (Type == Class::Object && hasKey(key)) {
  ...
  bool remove(const unsigned index) {
    if (Type == Class::Array && index < Internal.List->size()) {
  ...
  string ToString(bool &ok) const {
    ...
    //            return ok ? std::move( json_unescape( *Internal.String ) ):
    //            string("");
  }
...
JSON parse_object(const string &str, size_t &offset) {
  ...
  while (offset < str.length()) {
    ...
    offset++;
    consume_ws(str, offset);
    JSON Value = parse_next(str, offset); // Value
    ...
    if (offset < str.length() && str[offset] == ',') {
      ...
      if (offset >= str.length()) {
        throw std::runtime_error(
            "json parsing: Expected comma or '}', found EOF");
      }
      throw std::runtime_error(
          "json parsing: Expected comma or '}', found another");
    }
  }
...
JSON parse_array(const string &str, size_t &offset) {
  ...
  while (offset < str.length()) {
    ...
    if (offset < str.length() && str[offset] == ',') {
      ...
      if (offset >= str.length()) {
        throw std::runtime_error(
            "json parsing: Array: Expected ',' or ']', found EOF");
      }
      throw std::runtime_error(
          "json parsing: Array: Expected ',' or ']', found another");
    }
  }
...
JSON parse_string(const string &str, size_t &offset) {
  ...
  for (char c = str[offset]; offset < str.length() && c != '\"';
       c = str[offset]) {
    if (c == '\\') {
      offset++;
      if (offset >= str.length()) {
        throw std::runtime_error(
            "json parsing: String: Expected '\"', found EOF");
      }
      switch (str[offset]) {
      case '\"':
        val += '\"';
      ...
      case 'u': {
        ...
        if ((offset + 4) >= str.length()) {
          throw std::runtime_error(
              "json parsing: String: Expected 4 HEX, found EOF");
        }
        for (unsigned i = 1; i <= 4; ++i) {
          ...
          if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') ||
            ...
            throw std::runtime_error("json parsing: Expected hex character in "
                                     "unicode escape, found another");
          }
        }
        offset += 4;
      } break;
  ...
  if (c == 'E' || c == 'e') {
    ...
    while (offset < str.length()) {
      ...
      if (c >= '0' && c <= '9')
        ...
        if (offset >= str.length()) {
          throw std::runtime_error("json parsing: Number: Expected a number "
                                   "for exponent, found EOF");
        }
        throw std::runtime_error("json parsing: Number: Expected a number for "
                                 "exponent, found another");
      } else
        break;
    }
    ...
    if (offset >= str.length()) {
      throw std::runtime_error(
          "json parsing: Number: Expected a number for exponent, found EOF");
    }
    throw std::runtime_error(
        "json parsing: Number: Expected a number for exponent, found another");
  }
  offset--;
...
JSON parse_false(const string &str, size_t &offset) {
  ...
  if (str.substr(offset, 5) == "false")
    ...
    throw std::runtime_error(
        "json parsing: Bool: Expected 'false', found another");
  }
  offset += 5;
...
JSON parse_true(const string &str, size_t &offset) {
  ...
  if (str.substr(offset, 4) == "true")
    ...
    throw std::runtime_error(
        "json parsing: Bool: Expected 'true', found another");
  }
  offset += 4;
  return std::move(Bool);
}
...
JSON parse_null(const string &str, size_t &offset) {
  ...
  if (str.substr(offset, 4) != "null") {
    throw std::runtime_error(
        "json parsing: Null: Expected 'null', found another");
  }
  offset += 4;
